...
## Generated lockfile rule

`implemented/manifest.yml` is generated by `tools/rfc_lock.py`. Humans and Agents MUST NOT use it as an input declaration of implementation status.
...
The generator computes a BLAKE3 fingerprint for each normalized normative requirement, derives source/test/evidence bindings from annotations, verifies referenced paths, and preserves `verified_against` so only changed requirements become stale.
...
// @satisfies FAS-...
// @test FAS-...
// @evidence FAS-... Event.Name
// @not-applicable FAS-... condition=... reason=...
~~~

The descriptive `implementation/bindings.yml` file helps planning, but only the generated manifest has implementation-binding authority.
